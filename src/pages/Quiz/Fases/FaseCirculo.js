...
import premio from '../../../images/icons/quarto.svg'
...
export default function Quiz() {
  ...
  const[nome_emblema,setNomeEmblema]= useState("Forte");
  ...
  useEffect(() => {
    async function getQuestions() {
      ...
      try {
        const resposta = await api.get('/faseCirculo');
     ...
     items-center h-screen">
       ...
       {  questions.length > 0 ? 
          ...
          <CardContent className={classes.conteudo}>
            ...
            <Typography variant="h3" color="textSecondary" component="h5">
              Parabéns! Você concluiu a Fase do Círculo.
          ...
          />
           <Typography variant="h3" color="textSecondary" component="h5"className={classes.titulo}>
              Clique no Botão para adicionar o emblema Circulo!
            </Typography>
            <CardContent className={classes.conteudoEmblema}>
             <form onSubmit={adicionarEmblema} > 
                ...
                <Button type='submit' className={classes.bnt1}>Emblema Forte</Button>
              </form> 
            </CardContent>
          </Card>
          </Grid>
